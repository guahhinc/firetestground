document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        renderConversationHistory() {
            ...
            if (!otherUser) { conversationView.innerHTML = `<div id="conversation-placeholder"><p>Could not load conversation.</p></div>`; return; }
            ...
            conversationView.innerHTML = ` <div id="conversation-header"> <button id="back-to-convos-btn"><span class="material-symbols-rounded">arrow_back_ios_new</span></button> ${profileLink} </div> <div id="messages-list"></div> <form id="message-input-form"> <input type="text" id="message-input" placeholder="Type a message..." autocomplete="off" required> <button type="submit" class="primary">Send</button> </form> `;
            ...
            setTimeout(() => messagesListEl.scrollTop = messagesListEl.scrollHeight, 100);
        },
...
chevron_right
The issue is happening because `core.initializeApp` blindly catches **all** errors and treats them as "Session errors," forcing a logout. If the server or the app detects a ban during the refresh process and throws an error (or if a network hiccup occurs), the catch block executes `core.logout()`.

I have updated the `script.js` file. Specifically, I modified `core.initializeApp` to check if the error is a **Ban** before logging out. If it is a ban, it simply redirects to the ban screen and keeps you logged in.
